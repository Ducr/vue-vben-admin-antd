...
静态路由和动态路由都会走权限控制，可以通过配置路由的 `meta` 属性中的 `authority` 字段来控制权限，可以参考[路由权限控制](https://github.com/Ducr/vue-vben-admin-antd/blob/master/playground/src/router/routes/modules/demos.ts)。
